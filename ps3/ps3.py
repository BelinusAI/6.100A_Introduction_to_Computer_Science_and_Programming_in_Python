...
def get_word_score(word, n):
    ...
    second_component = max(7 * len(s) - 3 * (n - len(s)), 1)
...
def update_hand(hand, word):
    ...
    for i in word.lower():
        if(new_hand.get(i, False) and new_hand[i] > 0):
            new_hand[i] -=1
...
def is_valid_word(word, hand, word_list):
    ...
    returns: boolean
    """
    wildcard_index = word.lower().find('*')
    ...
    if (wildcard_index < 0):
        ...
        count = 0
        for i in VOWELS:
            if(word.lower()[:wildcard_index] + i + word.lower()[wildcard_index + 1:] in word_list):
                count += 1
        if count < 1:
            return False   

    test_word = get_frequency_dict(word.lower())
    for i in test_word.keys():
        if((i in hand and hand[i] < test_word[i] or i not in hand) and i != "*"):
            return False
    ...
    while(calculate_handlen(hand) > 0):
        ...
        if(word == '!!'):                                # If the input is two exclamation points
            ...
            if(is_valid_word(word, hand, word_list)):    # If the word is valid:
                word_score = get_word_score(word, calculate_handlen(hand))    # Tell the user how many points the word earned
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(number_of_hands):
        ...
        if(not repeat_done):
            ...
            if(repeat == "yes"):
                repeat_done = True
                hand_score = max(play_hand(hand, word_list), hand_score)
