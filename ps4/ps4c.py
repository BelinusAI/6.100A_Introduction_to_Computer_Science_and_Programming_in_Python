...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for i in permutations:
            enc_dict = self.build_transpose_dict(i)
            decrypt_message = self.apply_transpose(enc_dict).split()
            permutation_real_word = 0
            for word in decrypt_message:
                if(is_word(self.get_valid_words(), word)):
                    permutation_real_word += 1

            if (permutation_real_word > real_word):
                real_word = permutation_real_word
                permutation = i
             
        enc_dict = message.build_transpose_dict(permutation)
        return(permutation, self.apply_transpose(enc_dict))
